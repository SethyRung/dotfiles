...
5. As a developer, I want `dotfiles init`, `dotfiles doctor`, `dotfiles stow`, `dotfiles clean`, and `dotfiles sync` in v1 so that I can Bootstrap, check, re-link, clean backups, and sync config.
...
33. As a developer, I want `dotfiles` symlinked to `~/.local/bin/dotfiles` during init so that `dotfiles doctor` and `dotfiles stow` work from any directory. If that dest already exists (including a dangling symlink), replace it so init does not fail with EEXIST.
...
40. As a developer, I want `dotfiles stow` to apply the same backup-then-Stow rule and relink `~/.local/bin/dotfiles` so that I can re-link configs and the PATH stub without running full init.
...
50. As a developer reading `dotfiles --help` (or equivalent), I want init, doctor, stow, clean, and sync described so that I do not need the README to discover commands.
...
61. As a user whose maintainer pushed new config to the repo, I want `dotfiles sync` to pull the repo and re-Stow so that my machine's config matches the repo without re-running Bootstrap.
62. As a developer re-running `dotfiles stow` or `dotfiles sync` on a Bootstrapped machine, I want existing dests that are already symlinks refreshed without timestamped backups so that re-runs do not litter `$HOME` with stamps.
63. As a developer who moved this repo, I want `dotfiles stow` (run from the new location) to retarget `~/.local/bin/dotfiles` and Stow dests so Workflow config points here again.
...
- One user-facing module: the `dotfiles` CLI. Commands in v1: `init`, `doctor`, `stow`, `clean`, `sync`. No `package`, `link`, or completions.
...
- After successful init, symlink the bash stub to `~/.local/bin/dotfiles`, replacing an existing dest (file or symlink, including dangling). Curated zshrc puts `~/.local/bin` on PATH. `dotfiles stow` does the same PATH relink plus re-Stow of `home/`.
...
- The only module under test is the `dotfiles` CLI, with a fake Host injected at the Host seam. Cover init (fresh, idempotent continue/decline, API Key merge, Ghostty yes/no, unknown Distro, PATH dest replaced), doctor (missing vs present, no secret leakage), and stow (backup-then-link, skip junk files, PATH relink, stale dests replaced with no backup), and clean (no-op, confirm deletes, decline keeps files), and sync (pull then re-Stow, mirror refresh, pull failure exits non-zero).
...
- Naming the CLI `dot`
- `dotfiles update` as a command, and tool upgrades via a CLI command (brew/mas-style) — rejected by ADR 0014 and 0015; config sync is `dotfiles sync`
