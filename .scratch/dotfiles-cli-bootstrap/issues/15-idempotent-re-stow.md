...
**What to build:** `dotfiles stow` and `dotfiles update` skip the timestamped backup when the destination already symlinks into this repo's `home/` tree (a new Host `linksIntoRepo` check) and just re-link. Real files at destinations keep the story-39 backup-then-Stow rule. Re-runs on a Bootstrapped machine leave zero stamp files in `$HOME`. Tests observe backups through the fake Host.

**Blocked by:** 03: `dotfiles stow` backs up then links; 14: `dotfiles update` syncs config
...
- [x] `dotfiles update` on a Bootstrapped machine churns no backups
