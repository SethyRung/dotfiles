...
- [x] Stowed zsh completion lists `init`, `doctor`, `stow`, `clean`, and `sync`
- [x] Flag completion covers `--help`, `--dry-run`, `--yes`, `--json`, and `--version` only on the commands that accept them
- [x] Curated zshrc prepends the completions directory to `fpath` before Oh My Zsh
- [x] Top-level help does not list a `completions` command (nor `update`, `repair`, `package`, or `link`)
- [x] README mentions `--yes`, `--json`, `--version`, zsh completions, Grok/Codex snapshot, and Ghostty on zypper
